...
void App::RunTracking(float ratio) {
    ...
    if (!video->isOpened()) {
        ...
        while (true) {
            ...
            // Convert center to Box
            vector<TrackingBox> detFrameData = toBox(kps, 15.);
        ...
        // each prediction is corresponding to a tracker
            ...
            for (auto tb : frameTrackingResult) {
                ...
                cv::circle(videoFrame, toCenter(tb.box), 7, randColor[tb.id % CNUM], 7);
                cv::putText(videoFrame, (tb.id), toCenter(tb.box),
            }
